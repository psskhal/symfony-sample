...
preg_match("/srf_uPost='(.*?)'/", $page_result, $regs);
$post_url = $regs[1];
//---------------------------------------------------STEP 2

$postal_data ='idsbho=1&PwdPad=IfYouAreReadingThisYouHaveTooMu&LoginOptions=2&CS=&FedState=&PPSX=Pas&type=11&login='.$username.'&'.'passwd='.$password.'&remMe=1&NewUser=0&PPFT='.$found1.'&i1=1&i2=2';
$result =curl_post($post_url,$postal_data,1,0);
...
$url = $matches;
$matches =curl_get($url,1,0);
preg_match('/&n=(.*?)"/', $matches, $regs);
$result = $regs[1];

//---------------------------------------------------STEP 4 - OPEN ADDREDD CHERRY PICKER

$url = 'http://bl136w.blu136.mail.live.com/mail/GetContacts.aspx?n='.$result;
$result =curl_get($url,1,0);
@unlink ($path_to_cookie);
//------------------------------------------------------------------HOTMAIL - END -----------------------------------------------------------------
}
...
//---------------------------------------------------STEP 2
$postal_data = 'm_PR=27&m_CBURL=http://mail.lycos.com&m_U='.$username.'&m_P='.$password.'&m_AS=1&x=42&y=8';
...
$url=$matches2.'/bn/address.cgi?login='.$username.'&FormName=export&session_id='.$matches.'&FormName=group_book';
$result = curl_get($url,1,0);
